...
from native_sg.common.environment.ddp_utils import is_main_process
...
def get_requirements_path(requirements_file_name: str) -> Optional[Path]:
    ...
    Note: This is because when installed from artifact only the source code is accessible, so requirements.txt has to be
          copied to the package root (./src/native_sg). This is automatically done with the CI to make sure that
          in the github we only have 1 source of truth for requirements.txt. The consequence being that when the code
          is copied/cloned from github, the requirements.txt was not copied to the native_sg package root, so we
          need to go to the project root (.) to find it.
    """
    file_path = Path(__file__)  # Refers to: .../super-gradients/src/native_sg/sanity_check/env_sanity_check.py
    package_root = file_path.parent.parent  # Refers to: .../super-gradients/src/native_sg
...
def check_packages():
    ...
    if requirements is None:
        logger.info(msg='Library check is not supported when native_sg installed through "git+https://github.com/..." command')
        return
