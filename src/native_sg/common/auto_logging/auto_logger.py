...
class AutoLoggerConfig:
    ...
    def _setup_default_logging(self, log_level: str = None) -> None:
        ...
        self._setup_logging(
            filename=os.path.join(env_variables.NATIVE_SG_LOG_DIR, f"logs_{os.getppid()}_{timestamp}.log"),
    ...
    def get_instance(cls):
        global _native_sg_logger_config
        if _native_sg_logger_config is None:
            _native_sg_logger_config = cls()
            _native_sg_logger_config._setup_default_logging()

        return _native_sg_logger_config
