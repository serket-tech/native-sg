...
from native_sg.common.crash_handler.utils import indent_string, fmt_txt, json_str_to_dict
...
class SGLoggerIsNoneTip(CrashTip):
    ...
    def _get_tips(cls, exc_type: type, exc_value: Exception, exc_traceback: TracebackType) -> List[str]:
        tip = (
            ...
            "native_sg.common.environment.ddp_utils.multi_process_safe decorator, or check that "
...
class DDPNotInitializedTip(CrashTip):
    ...
    def _get_tips(cls, exc_type: type, exc_value: Exception, exc_traceback: TracebackType) -> List[str]:
        tip = (
            ...
            f">>> {fmt_txt('from native_sg.training.utils.distributed_training_utils import setup_device', color='green')}\n"
            f">>> {fmt_txt('from native_sg.common.data_types.enum import MultiGPUMode', color='green')}\n"
