...
class AutoTrainBatchSizeSelectionCallback(PreLaunchCallback):
    ...
    Example usage: Inside the main recipe .YAML file (for example native_sg/recipes/cifar10_resnet.yaml),
     add the following:
    ...
    Then, when running native_sg/examples/train_from_recipe_example/train_from_recipe.py --config-name=...
...
def modify_params_for_qat(
    ...
    :param quantization_params: Dict, quantization parameters as passed to Trainer.qat(...). When None, will use the
     default parameters in native_sg/recipes/quantization_params/default_quantization_params.yaml
...
class QATRecipeModificationCallback(PreLaunchCallback):
    ...
    Inside the main recipe .YAML file (for example native_sg/recipes/cifar10_resnet.yaml), add the following:
