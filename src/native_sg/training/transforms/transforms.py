...
from native_sg.common.factories.data_formats_factory import ConcatenatedTensorFormatFactory
from native_sg.common.factories.torch_dtype_factory import TorchDtypeFactory
from native_sg.common.object_names import Transforms, Processings
from native_sg.common.registry.registry import register_transform
from native_sg.training.datasets.data_formats import ConcatenatedTensorFormatConverter
from native_sg.training.datasets.data_formats.bbox_formats.xywh import xyxy_to_xywh
from native_sg.training.datasets.data_formats.default_formats import XYXY_LABEL, LABEL_CXCYWH
from native_sg.training.datasets.data_formats.formats import filter_on_bboxes, ConcatenatedTensorFormat
from native_sg.training.samples import DetectionSample, SegmentationSample
from native_sg.training.transforms.detection import DetectionPadIfNeeded, AbstractDetectionTransform, LegacyDetectionTransformMixin
from native_sg.training.transforms.segmentation.abstract_segmentation_transform import AbstractSegmentationTransform
from native_sg.training.transforms.segmentation.legacy_segmentation_transform_mixin import LegacySegmentationTransformMixin
...
from native_sg.training.utils.detection_utils import (
    get_mosaic_coordinate,
    adjust_box_anns,
    DetectionTargetsFormat,
    change_bbox_bounds_for_image_size_inplace,
)
from native_sg.training.utils.utils import ensure_is_tuple_of_two
...
class DetectionTargetsFormatTransform(AbstractDetectionTransform, LegacyDetectionTransformMixin):
    ...
    def __init__(
        ...
        if isinstance(input_format, DetectionTargetsFormat) or isinstance(output_format, DetectionTargetsFormat):
            raise TypeError(
                "DetectionTargetsFormat is not supported for input_format and output_format starting from native_sg==3.0.7.\n"
                "You can either:\n"
                "\t - use builtin format among native_sg.training.datasets.data_formats.default_formats.<FORMAT_NAME> (e.g. XYXY_LABEL, CXCY_LABEL, ..)\n"
                "\t - define your custom format using native_sg.training.datasets.data_formats.formats.ConcatenatedTensorFormat\n"
            )
